//
// Software Name: OUDS Android
// SPDX-FileCopyrightText: Copyright (c) Orange SA
// SPDX-License-Identifier: MIT
//
// This software is distributed under the MIT license,
// the text of which is available at https://opensource.org/license/MIT/
// or see the "LICENSE" file for more details.
//
// Software description: Android library of reusable graphical components
//

// Generated by Tokenator

package com.orange.ouds.theme

object OudsVersion {
    object Component {
        const val AlertMessage = "1.2.0"
        const val AppBar = "1.0.0"
        const val Badge = "1.2.0"
        const val BadgeCount = "1.2.0"
        const val BadgeIcon = "1.3.0"
        const val BottomSheet = "0.0.0"
        const val BulletList = "1.1.0"
        const val Button = "3.4.0"
        const val Checkbox = "2.4.0"
        const val CircularProgressIndicator = "1.0.0"
        const val Divider = "1.0.0"
        const val Fab = "0.0.0"
        const val FilterChip = "1.5.0"
        const val InlineAlert = "1.0.0"
        const val InputTag = "1.2.0"
        const val Link = "2.3.0"
        const val LinearProgressIndicator = "1.0.0"
        const val NavigationBar = "1.0.0"
        const val NavigationButton = "3.4.0"
        const val NavigationCardItem = "1.2.0"
        const val NavigationListItem = "1.2.0"
        const val PasswordInput = "1.3.1"
        const val PinCodeInput = "1.3.0"
        const val RadioButton = "1.4.0"
        const val StaticCardItem = "1.2.0"
        const val StaticListItem = "1.2.0"
        const val SuggestionChip = "1.5.0"
        const val Switch = "1.5.0"
        const val Tag = "1.5.0"
        const val TextArea = "1.2.1"
        const val TextInput = "1.4.1"
    }
    object Tokens {
        const val OudsCore = "1.11.0"
        const val AndroidCore = "1.0.0"
        const val AndroidSystem = "1.2.0"
        const val OrangeCore = "1.2.0"
        const val OrangeBrand = "2.7.0"
        const val OrangeCompactBrand = "2.7.0"
        const val SoshCore = "1.4.0"
        const val SoshBrand = "2.7.0"
        const val WireframeCore = "1.5.0"
        const val WireframeBrand = "2.7.0"
    }
}

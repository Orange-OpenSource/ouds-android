...
description: Use this skill when the user needs to write code using OUDS Android components, set up the library, configure themes, access design tokens, or fix OUDS issues in Kotlin/Compose. Includes adding dependencies, wrapping UI with OudsTheme, creating Ouds* components (OudsButton, OudsTextInput, OudsNavigationBar, OudsTag, OudsCheckbox, etc.), accessing tokens via OudsTheme.colorScheme/spaces/typography/borders, configuring fonts, handling tinted/untinted icons, using OudsColoredBox, showing errors with OudsError, switching themes, styling with component tokens, or migrating from Material 3 to OUDS. Trigger when user asks to 'create', 'show me how', 'write a composable', 'set up', 'access', 'fix', 'debug', 'implement', 'migrate', 'add', 'use', 'build', or 'style' anything related to OUDS components or tokens, even without mentioning 'OUDS framework' or 'design system'. Also trigger for examples, code samples, or UI questions in OUDS Android apps.
license: MIT
---

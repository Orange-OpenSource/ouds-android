...
private val LocalExtraParametersByClass =
    mutableMapOf<KClass<out OudsComponentContent.ExtraParameters>, ProvidableCompositionLocal<out OudsComponentContent.ExtraParameters>>()

internal fun <T> getLocalExtraParameters(clazz: KClass<T>): ProvidableCompositionLocal<T> where T : OudsComponentContent.ExtraParameters {
    return LocalExtraParametersByClass[clazz]?.asOrNull<ProvidableCompositionLocal<T>>().orElse {
        staticCompositionLocalOf<T> { error("CompositionLocal LocalExtraParameters for class ${clazz.qualifiedName} not present") }.also { compositionLocal ->
 ...
 * @suppress
 */
abstract class OudsComponentContent<T> internal constructor(private val extraParametersClass: KClass<T>) where T : OudsComponentContent.ExtraParameters {
